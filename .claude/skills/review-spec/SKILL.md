...
`NAME_CARRIES_PATH` (an api name repeating its collection path — nothing will match it, the router answers `Not Implemented`) and `LABEL_AS_PATH` (a non-`/` path on an ENABLED collection — it gets appended to a node-url) are HIGH-severity findings. `AMBIGUOUS_REST_NAME` is informational: report it, and check the PR says whether the author accepted it. Background: `create-spec/references/phase3.3-api-collections.md` → Step 3.3c.
