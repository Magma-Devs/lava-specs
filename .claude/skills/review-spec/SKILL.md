...
If any spec under review sets `internal_path`, run the internal-path guard in the same pass:

```bash
bash .claude/skills/create-spec/scripts/check_internal_paths.sh <spec.json> [more.json ...]
```

`NAME_CARRIES_PATH` (an api name repeating its collection path — nothing will match it, the router answers `Not Implemented`) and `LABEL_AS_PATH` (a non-`/` path on an ENABLED collection — it gets appended to a node-url) are HIGH-severity findings. `AMBIGUOUS_REST_NAME` is informational: report it, and check the PR says whether the author accepted it. `AMBIGUOUS_REST_SHAPE` — two names differing only inside their `{placeholders}`, which compile to one pattern — is informational too, but there is no trade to accept: the shadowed name should be dropped. Background: `create-spec/references/phase3.3-api-collections.md` → Step 3.3c.
