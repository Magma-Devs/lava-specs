...
- **Boot-critical presence.** `check_directive_presence.sh` BFS-walks the candidate's `imports` graph and requires `GET_BLOCKNUM` **and** `GET_BLOCK_BY_NUM` in the *union* of the candidate plus all transitive parents. Inheritance-aware by necessity: EVM L2s inherit from ETH1 and Cosmos chains from COSMOSSDK/TENDERMINT, so their own `parse_directives` arrays are empty by design and a naive check false-FAILs every one of them. Hard-FAILs only when a tag is absent everywhere.
